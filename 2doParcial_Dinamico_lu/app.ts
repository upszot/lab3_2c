...
let nuevaVeterinaria = new Veterinaria("pedro lopez");
...
function mascotaVacia() //como el corazon de tu ex
{
    localStorage.setItem('ultimoId', "4999");
    let mascotaVacia = new Legislador(ETipo, null, null, null, null, null, localStorage.ultimoId++);
    var mascotas = [];
...
function encabezadoGenerla(personas) {
  ...
  tabla.appendChild(Recursos.prototype.agregarLabel("% Femenino: ", "filtro"));
    ...
    let boton = Recursos.prototype.agregarBoton("Limpiar localStorage", "btn btn-danger");
    boton.addEventListener('click', function () {
        localStorage.clear();
        asignarManejadores();
    });
...
function promedioEdades(personas) {
    var contador = 1;
    var edadesTotales = personas.map((user) => {  
            return user.edad; 
    })
...
function cargarForm() {
    ...
    let mascotas = JSON.parse(localStorage.mascotas);
...
function arrayTipoAnimales() {
    let arr: ETipo[] = Object.keys(ETipo).map(k => ETipo[k]);
    var arraySinNUmeros = []
    for (var dato in arr) {
        if (isNaN(arr[dato])) {
